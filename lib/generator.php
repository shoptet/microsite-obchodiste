...
function generate_random_user():int {
...
function generate_random_wholesaler( $user_id ):int {
  ...
  $rand_term_id = get_random_term( $terms );
  $meta_input = [
    'is_shoptet' => rand( 0, 1 ),
...
function generate_wholeasers( $bunch ) {
  ...
  for( $i = 0; $i < $wholesalers_num; $i++ ) {
    ...
    $wholesaler_id = generate_random_wholesaler( $user_id );
    $wholesalers_by_user_id[$user_id] = $wholesaler_id;
    generate_random_products( $products_per_wholesaler, $wholesaler_id, $user_id );
  }
}

function generate_random_attachments( $attachments_num, $product_id ) {
...
function get_random_term( $terms ) {
  $random_term = $terms[array_rand($terms)];
  return $random_term;
}

function generate_random_products( $products_num, $wholesaler_id, $user_id ) {
  global $wpdb;
  $term_query = new WP_Term_Query( [
...
function generate_dummy_data(): void
{
  ...
  foreach( $config as $bunch ) {
    $wholesalers_by_user_id = generate_wholeasers( $bunch );
  }
